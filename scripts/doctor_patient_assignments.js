...
function makeTable(rowsArray) {
    const columns = ['Doctor Employee ID', 'Doctor First Name', 'Doctor Last Name', 'Patient Record Number', 'Patient First Name', 'Patient Last Name', 'Patient DOB', 'Unassign']
...
function makeTableBodyRows(rowsArray) {
    ...
    for (let i = 0; i < quantityOfRows; i++) { // make this many rows in the table
        const row = document.createElement('tr')
        for (let j = 0; j < 8; j++) {
            const currentRow = rowsArray[i] // an array of objects containing the row information that comes from a response from server
            if (j < 6) {
...
document.getElementById('makeAssignment').addEventListener('click', function (event) {
    let payload = { employeeID: null, patientRecordNumber: null }
...
document.getElementById('showDoctorsAssignments').addEventListener('click', function (event) {
    ...
    req.addEventListener('load', function () {
        if (req.status >= 200 && req.status < 400) {
            ...
            const databaseRowsArray = response.results
...
document.getElementById('showPatientsAssignments').addEventListener('click', function (event) {
    const req = new XMLHttpRequest();
    let payload = { patientRecordNumber: null }
...
const getDoctorInfo = () => {
    const req = new XMLHttpRequest();
    const firstDoctorSearchSelect = document.getElementById("assignDoctor");
    const secondDoctorSearchSelect = document.getElementById("showAssignedDoctor");
    req.open('GET', baseURL + 'get_doctor_names', true);
    req.addEventListener('load', function () {
        if (req.status >= 200 && req.status < 400) {
            const doctorInfo = JSON.parse(req.responseText)
...
const getPatientInfo = () => {
    ...
    const req = new XMLHttpRequest();
    const firstPatientSearchSelect = document.getElementById("assignPatient");
    const secondPatientSearchSelect = document.getElementById("showAssignedPatient");
    req.open('GET', baseURL + 'get_patient_names', true);
    req.addEventListener('load', function () {
        if (req.status >= 200 && req.status < 400) {
            const patientInfo = JSON.parse(req.responseText)
...
const populateDropDown = (objectWithRows, dropDownToPopulate) => {
    ...
    if (Object.is(document.getElementById("assignDoctor"), dropDownToPopulate) || Object.is(document.getElementById("showAssignedDoctor"), dropDownToPopulate)) {
        for (let i of objectWithRows.results) {
