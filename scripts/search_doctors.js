...
function makeTable(rowsArray) {
    const columns = ['Doctor Employee ID', 'First Name', 'Last Name', 'Specialty', 'Update', 'Delete']
...
document.getElementById('searchDoctor').addEventListener('click', function (event) {
    ...
    let payload = { firstName: null, lastName: null, specialty: null }
...
document.getElementById('updateSubmit').addEventListener('click', (event) => {
    let payload = { employeeID: null, firstName: '', lastName: '', specialty: '' };
...
function deleteRow(idOfRow) {
    const req = new XMLHttpRequest();
    let payload = { employeeID: idOfRow }
