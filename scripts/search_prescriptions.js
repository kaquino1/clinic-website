...
function makeTable(rowsArray) {
    const columns = ['Prescription ID', 'Employee ID', 'Doctor FN', 'Doctor LN', 'Medication ID', 'Medication Name', 'Quantity Prescribed', 'Patient Record #', 'Patient FN', 'Patient LN']
...
document.getElementById('searchPrescriptionSubmit').addEventListener('click', function (event) {
    ...
    let payload = { employeeID: null, patientRecordNumber: null, medID: null }
...
const getPatientInfo = () => {
    ...
    const req = new XMLHttpRequest();
    const searchSelect = document.getElementById("searchPatient");
...
const getMedicationInfo = () => {
    ...
    const req = new XMLHttpRequest();
    const searchSelect = document.getElementById("searchMedication");
    req.open('GET', baseURL + 'get_medication_names', true);
    req.addEventListener('load', function () {
        if (req.status >= 200 && req.status < 400) {
            const medicationInfo = JSON.parse(req.responseText)
